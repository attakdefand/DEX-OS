...
impl Authentication {
    ...
    pub fn authenticate(&self, username: &str, password: &str) -> Result<AuthSession, SecurityError> {
        let credentials = self.users.get(username)
...
impl Authorization {
    ...
    pub fn grant_permission(&mut self, user: &str, permission: &str) {
        let user_permissions = self.permissions.entry(user.to_string()).or_insert_with(Vec::new);
