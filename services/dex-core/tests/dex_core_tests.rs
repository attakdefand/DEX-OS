...
use dex_core::{DEXCore, Token, TokenPair, Order, OrderSide, OrderType, DEXError};
...
fn test_liquidity_pool_creation() {
    ...
    assert!(matches!(result.unwrap_err(), DEXError::LiquidityPoolError(_)));
...
fn test_liquidity_operations() {
    ...
    // Create pool
    dex.create_liquidity_pool(token_a.address, token_b.address, 30).unwrap();
    ...
    assert!(matches!(removed.unwrap_err(), DEXError::InsufficientLiquidity(_)));
}
...
fn test_token_swapping() {
    ...
    assert!(matches!(amount_out.unwrap_err(), DEXError::InsufficientLiquidity(_)));
    ...
    assert!(amount_out.is_err());
    assert!(matches!(amount_out.unwrap_err(), DEXError::InvalidTokenPair(_)));
}
...
fn test_edge_cases() {
    ...
    assert!(result.is_err());
    assert!(matches!(result.unwrap_err(), DEXError::InsufficientLiquidity(_)));
}
